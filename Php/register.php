<?php
    ...
    if(strcmp($password,$conpassword)==0)
    {
        ...
        else
        {
            ...
            $query3="CREATE TABLE `wallets`.`$wallet_name` ( `S. No.` INT NOT NULL AUTO_INCREMENT PRIMARY KEY , `Category` VARCHAR(10) NOT NULL , `Amount` INT NOT NULL, `Sub Category` VARCHAR(20) NOT NULL, `Date` DATE NOT NULL, `Description` VARCHAR(70) NOT NULL, `Mode` VARCHAR(15) NOT NULL )";
            $ins=mysqli_query($con,$query1);
            $crewal=mysqli_query($con,$query3);
            $_SESSION['wallet_name']=$wallet_name;
